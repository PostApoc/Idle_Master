...
def mainProgram():
    ...
    logging.basicConfig(filename="idlemaster-mod.log",filemode="w",format="[ %(asctime)s ] %(message)s", datefmt="%m/%d/%Y %I:%M:%S %p",level=logging.DEBUG)
    ...
    if sys.platform.startswith('win32'):
        ctypes.windll.kernel32.SetConsoleTitleA("Idle Master-Mod")

    logging.warning(Fore.GREEN + "WELCOME TO IDLE MASTER-MOD" + Fore.RESET)
...
def chillOut(appID, myProfileURL):
